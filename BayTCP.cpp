...
void BayTCPInterface::setConfig(const char* name,const char* host,const char* port,const char* path, const char* user, const char* pw){
	strcpy(_config_buffer,host);
	uint8_t offset=strlen(host)+1;
	strcpy(_config_buffer+offset,port);
	offset+=strlen(port)+1;
	strcpy(_config_buffer+offset,path);
	offset+=strlen(path)+1;
	strcpy(_config_buffer+offset,user);
	offset+=strlen(user)+1;
	strcpy(_config_buffer+offset,pw);
	offset+=strlen(pw)+1;
	strcpy(_config_buffer+offset,name);
	offset+=strlen(name)+1;
	_config_buffer[offset]=0;
	_config_buffer[offset+1]=0;
	_config_buffer[offset+2]=0;
	_config_buffer[offset+3]=0;
	setConfigPointers();
}
