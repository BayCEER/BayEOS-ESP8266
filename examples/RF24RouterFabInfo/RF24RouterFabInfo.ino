...
void setup(void) {
  P.begin();
  P.setIntensity(8);
  P.displayText("WIFI-Setup", PA_CENTER, 0, 0, PA_PRINT, PA_NO_EFFECT);
  P.displayAnimate();

  Serial.begin(115200);
  EEPROM.begin(2048);
  Serial.println();
  Serial.println("RF24 CE, CS");
  Serial.println(RF24_CE);
  Serial.println(RF24_CS);
 ...
 uint8_t reset_storage = 0;
  ...
  String t = "Connected to WIFI with IP ";
  t += WiFi.localIP().toString();
  t.toCharArray(text_buffer, 500);
  P.displayScroll(text_buffer, PA_LEFT, scrollEffect, frameDelay);
